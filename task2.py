...
"""
windowSpecPart2 = Window.partitionBy("location").orderBy(F.col("new_cases").desc(), F.col("date").asc())

df_task2_part2 = (df
    .filter(F.col("continent").isNotNull())
    .filter(F.col("date").between("2021-03-25", "2021-03-31"))
    .withColumn("rn", F.row_number().over(windowSpecPart2))
    .filter(F.col("rn") == 1)
    .select(
    ...
    # 5. Сортируем уже сами страны между собой и берем ТОП-10
...
"""
windowSpecPart3 = Window.partitionBy("location").orderBy("date")

df_task2_part3 = (df
    .filter(F.col("location") == "Russia")
    .filter(F.col("date").between("2021-03-24", "2021-03-31"))
    .withColumn("вчера", F.lag("new_cases").over(windowSpecPart3))
